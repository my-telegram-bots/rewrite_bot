...
- Group mode defaults to `replace`: the original is deleted and the bot republishes the cleaned content, so the visible sender becomes the bot. If deletion is unavailable, the original and replacement are retained without an additional warning or recovery message.
