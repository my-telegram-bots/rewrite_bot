...
- Original photos and Telegram-compatible progressive MP4/H.264 videos become native inline media results ahead of link-only variants. Selecting one sends a Telegram photo or video that the recipient can download. Multi-photo X/Twitter posts additionally expose FxTwitter's JPEG mosaic as an explicitly labeled combined-image result without replacing any selectable original item; external player cards, HLS playlists, and incompatible video codecs are not substituted for originals.
- For directly sent links, compatible media is sent as a reply to the original message; two or more photos/videos use a Telegram media group while animations and mixed unsupported album combinations retain source order as individual replies. The original user message is retained regardless of URL-cleaning group mode.
- Inline and direct media captions share one stable layout: the post body is a Telegram `blockquote`, followed by an author line and a localized `View original post` text link. The exact `U+200C` processed marker remains the first code point and all entity offsets use Telegram UTF-16 units.
...
- Tests cover exact social-post URL recognition, fixed API endpoints, timeout/redirect/body limits, malformed and failed payloads, media ordering, trusted FxTwitter mosaic selection without loss of originals, compatible video selection, persistent default-on settings and administrator callbacks, UTF-16 caption entities, native direct-message delivery, localized failure results, and real grammY inline-query output construction.
