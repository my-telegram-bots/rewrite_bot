...
If deletion fails, both the original and clean reply are kept without an additional warning.
...
When an inline query or directly sent text message contains an exact X/Twitter or Bluesky post URL, the bot asks the public FxEmbed API v2 for post metadata. Each compatible original photo or progressive MP4/H.264 video is offered as native Telegram media. Multi-photo X/Twitter inline results put FxTwitter's labeled combined JPEG first while retaining every full-quality original. A valid post with no top-level media is treated as a normal text-only result: direct handling sends no diagnostic and retains the original, while still applying ordinary URL cleanup if the URL changed; inline handling keeps its link and utility results and explains that the post may simply be text-only. Media from quoted posts or link previews is not forwarded as though it belonged to the requested post. Directly sent links with media receive native media; multi-photo/video posts use an album. In a group using `replace`, the bot sends the replacement media first and then deletes the original link. If deletion is unavailable, it keeps both messages without an additional warning; lookup and media-send failures never delete the original. Captions quote the post body, link the author handle to its Twitter or Bluesky profile, and provide a localized original-post link. The bot does not proxy the full media file through its own memory or disk.

The same persistent social-media parsing switch appears in personal and group `/settings` and defaults on. A separate multi-image setting has exactly two modes: `Media group` (default) sends every original photo as a Telegram album, while `Combine` sends FxTwitter's combined JPEG when the API provides one and otherwise falls back to the album. A personal switch also controls that user's inline results. Group changes require administrator status; disabling parsing suppresses FxEmbed calls without disabling ordinary URL cleanup.
